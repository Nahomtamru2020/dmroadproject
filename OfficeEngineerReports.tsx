...
export function OfficeEngineerReports({ onReportClick }: { onReportClick: (report: any, collection: string, editMode?: boolean) => void }) {
  const [reports, setReports] = useState<any[]>([]);

  useEffect(() => {
    // Assuming the collection is 'documents' and it has a field 'authorRole'
    const q = query(collection(db, 'documents'), where('authorRole', '==', 'Office Engineer'), orderBy('createdAt', 'desc'));
  ...
  const handleExport = () => {
    exportToCSV(reports, 'OfficeEngineerReports', ['Date', 'Content'], ['date', 'content']);
  };

  return (
    <div className="bg-white p-6 shadow-sm border border-slate-200 flex-1">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xs font-bold text-slate-500 uppercase">Office Engineer Reports</h2>
